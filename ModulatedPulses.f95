! Module to contains all pulses 
! for example Gaussian or modulated pulses
! A global pointer defined in the Parameters.f95 for wave packet to be used in the simulation
...
contains
    ...
    function modulated_pulse(kn, theta) result(final_pulse)
        ...
        ! exponential factor containing the bunch time which defines the length the wave packet has propagated
    ...
    function gaussian_pulse(kn, theta) result(final_pulse)
        ...
        ! Any propagation phase exponential is added by hardcoding the Tmatrix.f95 module (see line)
    ...
    function side_band(N, klong) result(band)
        ! Evaluation of a longitudinal k-value in the N'th side band and multiplied by the weight from the Bessel function
        integer, intent(in) :: N ! side band
        real(wp), intent(in) :: klong ! input k-value
        real(wp) :: band         ! result
        real(wp) :: bandcenter   ! central k-value of band (= k_0 + N*dk)
